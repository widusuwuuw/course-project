...
// 肾功能指标列表 - 使用后端返回的完整英文名称
const KIDNEY_FUNCTION_METRICS = [
  'Creatinine',
  'Blood Urea Nitrogen',
  'Cystatin C',
...
interface KidneyFunctionScreenProps {
  onBack: () => void;
  onAnalysisComplete: (results: any, values: {[key: string]: string}) => void;
}

export default function KidneyFunctionScreen({ onBack, onAnalysisComplete }: KidneyFunctionScreenProps) {
  ...
  useEffect(() => {
    const loadMetrics = async () => {
      try {
        ...
        console.log('🔄 Loading kidney function metrics...');
        const metrics = await getAvailableLabMetrics();
        console.log('📋 All available metrics:', metrics);
        console.log('🎯 Kidney function metrics expected:', KIDNEY_FUNCTION_METRICS);

        const kidneyMetrics = metrics.filter(metric =>
          KIDNEY_FUNCTION_METRICS.includes(metric.name_en)
        );
        console.log('✅ Filtered kidney metrics:', kidneyMetrics);

        setAvailableMetrics(kidneyMetrics);
      } catch (error) {
        console.error('❌ Failed to load kidney metrics:', error);
  ...
  const getMetricIcon = (metricName: string): string => {
    const iconMap: {[key: string]: string} = {
      'Creatinine': 'water-outline',
      'Blood Urea Nitrogen': 'water-outline',
      'Cystatin C': 'water-outline',
      'Estimated Glomerular Filtration Rate': 'water-outline',
      'Microalbumin': 'water-outline',
      'Urine Protein-to-Creatinine Ratio': 'water-outline',
      'Urea': 'water-outline',
      'Uric Acid': 'water-outline'
    };
  ...
  const handleAnalyze = async () => {
    ...
    console.log('🔍 Kidney Function Input metrics:', metricValues);
    console.log('✅ Kidney Function Filled metrics:', filledMetrics);
    ...
    try {
      ...
      console.error('❌ Kidney Function Analysis failed:', error);
    ...
    return (
      <View style={styles.container}>
        <View style={styles.header}>
          ...
          <Text style={styles.title}>肾功能检测</Text>
        </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.infoCard}>
          <View style={styles.infoHeader}>
            ...
            <Text style={styles.infoTitle}>肾功能8项检测</Text>
          </View>
          <Text style={styles.infoDescription}>
            包含肌酐、尿素氮、胱抑素C、肾小球滤过率等关键肾功能指标，用于评估肾脏健康状况和功能。
          </Text>
        </View>
        ...
        >
          {loading ? (
            ...
            <>
              ...
              <Text style={styles.analyzeButtonText}>分析肾功能结果</Text>
            </>
          )}
        </TouchableOpacity>
      </ScrollView>
