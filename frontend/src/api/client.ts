...
import { Platform } from 'react-native'; // Import Platform is now here
...
export async function uploadImage(uri: string, mimeType: string) {
  ...
  const filename = uri.split('/').pop(); // 从 URI 获取文件名

  // For React Native (iOS/Android): directly use the { uri, name, type } object
  // For React Native Web: fetch the URI first to get a Blob, then append the Blob
  if (Platform.OS === 'web') {
    const response = await fetch(uri);
    const blob = await response.blob();
    formData.append('file', blob, filename); // Append blob directly with filename
  ...
  if (!res.ok) throw new Error(await res.text());
  const data = await res.json();
  // Prepend API_BASE_URL to the relative URL received from the backend
