from fastapi import APIRouter, HTTPException
from pydantic import BaseModel, Field
from typing import Optional, Literal
import os
from .services.llm_client import generate_answer, LLMUnavailable, is_enabled as llm_enabled

DISCLAIMER = (
    "本助手仅提供一般性的健康科普与生活方式建议，不能替代医生的诊断或处方。"
    "如有紧急或严重不适，请及时就医。"
)

SENSITIVE_KEYWORDS = [
    # 诊断类
    "诊断", "确诊", "是不是得了", "我得了什么", "怎么判断是不是",
    # 用药/剂量类
    "处方", "药方", "吃什么药", "用药", "剂量", "mg", "毫克", "几片",
]


class UserProfile(BaseModel):
    sex: Optional[Literal["male", "female", "other"]] = None
    age: Optional[int] = Field(default=None, ge=0, le=120)


class Message(BaseModel):
    role: Literal["user", "assistant"]
    content: str

class AssistantQuery(BaseModel):
    question: str = Field(min_length=1, max_length=1000)
    question_type: Optional[
        Literal["general", "lifestyle", "diet", "sleep", "symptom"]
    ] = "general"
    user_profile: Optional[UserProfile] = None
    messages: Optional[list[Message]] = None  # Conversation history


class AssistantAnswer(BaseModel):
    answer: str
    disclaimer: str = DISCLAIMER
    refused: bool = False
    source: str = "template"  # template | llm
    image_url: Optional[str] = None


router = APIRouter(prefix="/assistant", tags=["assistant"])


def contains_sensitive(text: str) -> bool:
    t = text.lower()
    return any(kw.lower() in t for kw in SENSITIVE_KEYWORDS)


def get_specialized_prompt(question_type: str) -> str:
    """根据问题类型返回专属的系统提示词"""
    
    base_rules = """
【严格禁止】
- 绝不提供疾病诊断、药物处方、具体剂量
- 不推荐未经验证的偏方或极端方法
- 涉及医疗风险时，必须明确建议就医

【回答格式】
💡 核心建议：（1-2句话总结）
📋 具体方案：（分点列出可执行步骤）
⚠️ 注意事项：（安全提醒和个性化调整建议）

请用中文回答，保持专业、友好、实用。"""

    prompts = {
        "diet": f"""你是一位注册营养师，专注于体重管理和科学营养指导。

【专业领域】
- 三大营养素平衡（碳水/蛋白质/脂肪比例）
- 每日热量需求计算（基础代谢率+活动消耗）
- 食材选择与搭配（参考《中国居民膳食指南》）
- 餐次分配与进食时间优化
- 特殊人群营养需求（如素食者、乳糖不耐受）

【回答重点】
1. 提供具体食谱示例和营养素配比
2. 计算热量缺口（减重）或盈余（增重）
3. 推荐优质食材和烹饪方法
4. 强调循序渐进，避免极端节食

{base_rules}""",

        "sleep": f"""你是一位睡眠健康专家，精通睡眠医学和昼夜节律调节。

【专业领域】
- 睡眠卫生习惯建立（环境、温度、光线、噪音）
- 昼夜节律调节（褪黑素分泌、蓝光管理）
- 睡前放松技巧（渐进式肌肉放松、呼吸训练）
- 作息时间优化（入睡时间、起床时间、午睡管理）
- 睡眠障碍识别（失眠、打鼾、日间嗜睡）

【回答重点】
1. 提供可执行的睡眠时间表
2. 推荐助眠环境改造方案
3. 区分短期失眠vs慢性睡眠障碍
4. 强调"红色警报"症状需就医（如严重打鼾、呼吸暂停）

{base_rules}""",

        "lifestyle": f"""你是一位健康生活方式教练，擅长习惯养成和行为改变。

【专业领域】
- 运动计划设计（有氧+力量+柔韧性）
- 习惯养成策略（21天法则、微习惯）
- 压力管理与情绪调节
- 时间管理与作息优化
- 社交与心理健康平衡

【回答重点】
1. 设计阶梯式行动计划（从易到难）
2. 提供习惯追踪清单和打卡建议
3. 强调可持续性和长期坚持
4. 结合心理学原理（奖励机制、环境设计）

{base_rules}""",

        "symptom": f"""你是一位健康科普顾问，专注于症状识别和就医指导。

【核心原则】
⚠️ 你不能诊断疾病，只能提供：
1. 症状的可能原因科普（多种可能性，不做判断）
2. 自我缓解的一般方法（非治疗）
3. 就医紧急程度判断（立即/尽快/观察）
4. 就医前准备建议（记录症状、携带资料）

【红色警报症状】（需立即就医）
- 胸痛/呼吸困难/严重头痛
- 大量出血/意识改变/持续高热
- 剧烈腹痛/突发视力/听力丧失

【回答重点】
1. 列出可能原因（3-5种常见情况）
2. 提供缓解建议（非药物为主）
3. 明确就医时机和科室选择
4. 强调"仅供参考，不能替代诊断"

{base_rules}""",

        "general": f"""你是一位专业的健康管理顾问，专注于体重管理、营养指导、运动规划和生活方式优化。

【核心原则】
1. 基于循证医学和营养学，提供科学、实用的建议
2. 针对体重管理场景，给出可操作的具体方案（如热量控制、运动计划）
3. 回答结构化：先总结要点，再展开详细建议，最后提醒注意事项
4. 语气亲切专业，用通俗易懂的语言解释专业概念

{base_rules}"""
    }
    
    return prompts.get(question_type, prompts["general"])


@router.post("/query", response_model=AssistantAnswer)
def query(payload: AssistantQuery) -> AssistantAnswer:
    q = payload.question.strip()

    # 敏感词过滤：诊断/处方/剂量等直接拒绝
    if contains_sensitive(q):
        return AssistantAnswer(
            refused=True,
            answer=(
                "抱歉，我无法提供诊断或用药相关建议。"
                "建议咨询正规医疗机构或专业医生。"
            ),
        )

    # 如果已配置 LLM，尝试调用真实模型
    if llm_enabled():
        try:
            # 根据问题类型获取专属提示词
            system_prompt_content = get_specialized_prompt(payload.question_type or "general")
            
            llm_messages = [{"role": "system", "content": system_prompt_content}]
            
            # Add conversation history
            if payload.messages:
                for msg in payload.messages:
                    llm_messages.append({"role": msg.role, "content": msg.content})
            
            # Add current question
            llm_messages.append({"role": "user", "content": q})

            answer_text = generate_answer(llm_messages) # This call will need to be updated in llm_client.py
            return AssistantAnswer(answer=answer_text, source="llm")
        except LLMUnavailable:
            pass
        except Exception as e:
            # 后备：记录异常（这里简单忽略，可扩展到日志）
            fallback_note = f"(LLM调用失败，使用模板回答: {type(e).__name__})"
    else:
        fallback_note = "(未配置真实模型，使用模板回答)"

    # 回退模板逻辑
    type_templates = {
        "diet": "**营养建议：**\n\n- 保持三餐规律，_适当控制热量摄入_。\n- 增加**蔬果**和优质蛋白，减少精制糖和饱和脂肪。\n- 建议每日饮用 **8 杯水**。",
        # Added for rich media demo:
        "diet_image": {
            "answer": "**健康饮食小贴士：**\n\n均衡膳食是健康基石！",
            "image_url": "https://img.freepik.com/free-vector/hand-drawn-healthy-food-background_23-2148767980.jpg?w=740&t=st=1700000000~exp=1700000600~hmac=a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9b0c1d2e3f4a5b6c7d8e9f0a1b2"
        },
        "sleep": "**睡眠建议：**\n\n1. 保持固定的作息时间。\n2. 睡前 _1 小时_ 避免电子屏幕。\n3. 创造**安静、黑暗、凉爽**的睡眠环境。",
        "lifestyle": "生活方式建议：建立规律作息、坚持适量运动（每周150分钟中等强度）、保持良好心态。",
        "symptom": "症状咨询提醒：我无法诊断疾病，建议详细记录症状（时间、程度、诱因），及时咨询专业医生。",
        "general": "以下为一般科普信息，请结合自身情况，若持续不适或出现警示症状请及时就医。"
    }
    
    # Check for rich media template trigger
    # If user asks for "image" or "图片" in diet category
    if ("image" in q.lower() or "图片" in q.lower()) and payload.question_type == "diet":
        template_response = type_templates.get("diet_image")
        answer_text = template_response["answer"]
        image_url = template_response["image_url"]
        return AssistantAnswer(answer=answer_text, source="template", image_url=image_url)
    
    tip = type_templates.get(payload.question_type, type_templates["general"])
    answer = f"{tip}\n\n你的问题：{q}\n{fallback_note}" if 'fallback_note' in locals() else f"{tip}\n\n你的问题：{q}"
    return AssistantAnswer(answer=answer, source="template")
